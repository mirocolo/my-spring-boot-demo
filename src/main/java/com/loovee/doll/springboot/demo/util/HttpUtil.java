...
/**
 * Function: apacheHttpClient连接池工具类
 *
 * @author HeXin
...
public class HttpUtil {
	...
	public static String getUrlContent(String urlString, Map<String, String>... headers) {
		...
		try {
			...
			httpGet = getHttpGetByUrlString(urlString);
	...
	private static HttpGet getHttpGetByUrlString(String urlString) throws MalformedURLException, URISyntaxException {
	...
	public static String getForEntity(String urlString,Map<String,String> params,Map<String, String>... headers){
		String html = "";
		HttpGet httpGet = null;
		urlString = urlString.trim();
		if (null == urlString || urlString.isEmpty() || !urlString.startsWith("http")) {
		...
		try {
			// 转化String url为URI,解决url中包含特殊字符的情况
			httpGet = getHttpGetByUriBuilder(urlString,params);
			setCommonHeaders(httpGet);
			// 额外的header
			if (headers != null && headers.length > 0) {
				for (Map.Entry<String, String> header : headers[0].entrySet()) {
	...
	private static HttpGet getHttpGetByUriBuilder(String url, Map<String, String> params) throws URISyntaxException {
		URIBuilder ub = new URIBuilder();
		ub.setPath(url);
		ArrayList<NameValuePair> pairs = covertParams2BNVPS(params);
	...
	public static String postForEntity(String urlString, Map<String, String> params, Map<String, String>... headers) {
		...
		try {
			...
			if (params != null && params.size() > 0) {
				List<NameValuePair> nvps = covertParams2BNVPS(params);
	...
	private static ArrayList<NameValuePair> covertParams2BNVPS(Map<String, String> params) {
